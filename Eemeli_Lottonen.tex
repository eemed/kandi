...
\newcommand*{\Kuukausi}{\ifcase\month\or{}Tammi\or{}Helmi\or{}Maalis\or{}Huhti\or{}Touko\or{}Kesä\or{}Heinä\or{}Elo\or{}Syys\or{}Loka\or{}Marras\or{}Joulu\fi kuu}
    ...
    Useat kommunikointijärjestelmät käyttävät binäärijärjestelmää. Binäärijärjestelmässä mikä tahansa numero voidaan esittää muodossa, jossa on vain nollia ja ykkösiä. Esimerkiksi numero 9 on binäärijärjestelmässä bittijono 1001. Numeroita $n \in \left\{ 0, 1 \right\} = \Zset_2$ kutsutaan biteiksi.
    ...
    \begin{esimerkki}\label{esim:3bitsanatjatko}
        Koodataan sana 101 samaan tapaan kuin \hyperref[esim:3bitsanat]{esimerkissä~\ref{esim:3bitsanat}}. Lisätään neljäsbitti $x_4 = (x_1 + x_2 + x_3) \mod2$, joten koodattu sana on 1010. Jatketaan koodausta vielä toistamalla koodattu sana uudelleen eli sanan 101 lopullinen koodattu muoto on 10101010. Oletetaan, että välityksessä tapahtuu yksi virhe. Nyt vastaanottimessa viesti paloittellaan 4 bitin koodisanoihin. \hyperref[esim:3bitsanat]{Esimerkin~\ref{esim:3bitsanat}} mukaan tiedetään kumpi koodisanoista on väärä ja täten osataan myös korjata virheen.
    \end{esimerkki}
    ...
    \begin{maaritelma}[{\cite[s.~491]{PA}}]
        ...
        \begin{enumerate}
            \item Alkiota $u \in  A^n = \underbrace{A \times \cdots \times A}_{\text{n kappaletta}}$ kutsutaan \emph{sanaksi}. Sanan $u$ pituus on $n$ ja se muodostuu aakkoston $A$ alkioista.
    ...
    \begin{maaritelma}[{\cite[s.~492]{PA}}]\label{maar:perus}
        ...
        \begin{enumerate}
            ...
            \item\label{kht:paino} Hamming painolla tarkoitetaan sanan $u$ niiden komponenttien lukumäärää, jotka eroavat nollasta. Hamming painoa merkitään $wt\left(u\right)$.
    ...
    \begin{lause}[{\cite[s.~492]{PA}}]\label{lause:Hamming}
        ...
        \begin{enumerate}
            ...
            \item\label{kht:etaisyyspaino} $d\left(u, v\right) = wt\left(u - v\right)$
    ...
    \begin{proof}\cite[s.~492]{PA}\label{tod: Hamming}
        \begin{enumerate}
            \item Tämä seuraa suoraan \hyperref[maar:perus]{määritelmän~\ref{maar:perus}} kohdista~\ref{kht:etaisyys} ja~\ref{kht:paino}.
            \item Sanan $u - v$ komponentti kohdassa $i$ on 1, jos sanat $u$ ja $v$ eroavat komponentissa~$i$. Nyt määritelmän perusteella $wt\left(u - v\right) = d\left(u, v\right)$.
            \item Tämä seuraa suoraan \hyperref[maar:perus]{määritelmän~\ref{maar:perus}} kohdasta~\ref{kht:etaisyys}.
            \item Sanojen $u$ ja $v$ etäisyys $d\left(u, v\right) = 0$ jos sanojen $u$ ja $v$ kaikki komponentit kohdassa $i$ ovat samoja, kun $1 \le i \le n$.
            \item Olkoon $x = u - v$ ja $y = v - w$. Nyt lauseen~\ref{lause:Hamming} kohdan~\ref{kht:etaisyyspaino} nojalla epäyhtälön vasemmasta puolesta saadaan
                \[
                    d\left(u,w\right) = wt\left(u - w\right) = wt\left(x + y\right) \text{.}
                \]
                Samoin oikeasta puolesta saadaan
                \begin{gather*}
                    d\left(u,v\right) = wt\left(u - v\right) = wt\left(x\right) \text{~ja} \\
                    d\left(v,w\right) = wt\left(v - w\right) = wt\left(y\right) \text{.}
                \end{gather*}
                Todistettava epäyhtälö on siis
                \[
                    wt\left(x + y\right) \le wt\left(x\right) + wt\left(y\right) \text{.}
                \]
                Olkoon $x_i$ ja $y_i$ sanojen $x$ ja $y$ kohdassa $i$ olevia komponentteja. Olkoon myös sana $z = x + y$ ja sen kohdassa $i$ oleva komponentti $z_i$. Huomaamme, että  
                \[
                    wt\left(x + y\right) = \sum_{i = 1}^n z_i \qquad wt\left(x\right) = \sum_{i =1}^n x_i \qquad wt\left(y\right) = \sum_{i = 1}^n y_i \text{.}
                \]
                Riittää siis todistaa, että $z_i \le x_i + y_i$ kaikilla $1 \le i \le n$. Tämä seuraa kuitenkin suoraan siitä että $z_i = x_i + y_i \mod 2$.
        \end{enumerate}
    \end{proof}
    ...
    \begin{esimerkki}
        ...
        \begin{center}
            ...
            \end{tabular}
        \end{center}
        Tarkastellaan seuraavaksi sanojen etäisyyttä toisistaan. Valitaan ensin sanat joiden etäisyys toisistaan on 1. Valitaan esimerkiksi sanat $110$ ja $100$. Kutsutaan niiden vastaavia koodisanoja $u = 110011 \in C$ ja $v = 100110 \in C$. Nyt alkuperäiset sanat eroavat vain yhdessä komponentissa, mutta niiden koodisanat eroavat kolmessa komponentissa eli $d\left(u,v\right) = 3$. Yleisesti jos kaksi sanaa eroavat vain komponentissa $i$, niin komponentti $i$ esiintyy kahdessa funktiossa, jotka määrittelimme koodin $C$ tarkastuskomponenteille $x_4, x_5$ ja $x_6$. Koodisanojen etäisyys on siis vähintään 3.
        Tarkastellaan sitten sanoja joiden etäisyys on 2. Valitaan sanat $110$ ja $101$. Olkoon niiden vastaavat koodisanat $u = 110011 \in C$ ja $v = 101101 \in C$, joiden etäisyys on $d\left(u, v\right) = 4$. Yleisesti jos sanat eroavat komponenteissa $i$ ja $j$, niin kahdessa funktioista, jotka määrittelimme komponenteille $x_4x_5x_6$ esiintyy komponentti $i$, mutta ei komponenttia $j$ tai komponentti $j$, mutta ei komponenttia $i$.
    \end{esimerkki}
